var ThreeSixty = (function (window) {
    ...
    return function (container, options, realContainer) {
        var self = this;
        var index = 0;
        ...
        realContainer.style.width = options.width + 'px';
        realContainer.style.height = options.height + 'px';
        realContainer.style.backgroundImage = 'url("' + options.image + '")';
        realContainer.style.backgroundPosition = '0 0';
        ...
        self.update = function (xmove) {
            if(!xmove) {
                realContainer.style.backgroundPositionX = -(index % options.perRow) * options.width + 'px';
                console.log('x-index: ', index, realContainer.style.backgroundPositionX);
                // container.style.backgroundPositionY = container.style.backgroundPositionY;
            }
            realContainer.style.backgroundPositionY = -Math.floor(index / options.perRow) * options.height + 'px';
            console.log('y-index: ', index, realContainer.style.backgroundPositionY);
            // console.log('yContainer: ', container.style.backgroundPositionY);
            document.getElementById('zoomImage').src = options.imagesPath + index + '.png';
            // wheelzoom(getElementById('zoomer'));
        };
